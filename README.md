...
4. **필수 2차원 리스트(Inspector 리스트) 세팅**
   - `GameUIController`에는 직렬화된 리스트 2종이 있으며, 각각 "행" 구조(버튼 + 텍스트)를 가지므로 **Inspector에서 2열짜리 테이블을 채운다**고 생각하면 쉽습니다.

   | 리스트 이름 | 행 개수 예시 | 열1 (필수) | 열2 (선택) | 설명 |
   | --- | --- | --- | --- | --- |
   | `Target Button Bindings` | 2행 (`=20`, `=1`) | Target 버튼 오브젝트 | 버튼 하위 TextMeshProUGUI (없으면 자동으로 탐색) | 타겟 선택 버튼을 왼쪽부터 순서대로 넣습니다. 라벨을 비워두면 자동으로 `=값` 텍스트로 갱신됩니다. |
   | `Operator Button Bindings` | 3행 (`+`, `-`, `÷`) | 각 연산자 버튼 | _(비워둬도 됨)_ | 기본 연산자 3종(덧셈, 뺄셈, 나눗셈)의 버튼을 추가하고, 각 행의 `Operator Type` 드롭다운을 해당 기호로 지정합니다. |

   - 특수 버튼(×, √)은 리스트가 아닌 별도 필드(`multiplyButton`, `sqrtButton`)에 직접 연결합니다.
   - 리스트 길이를 늘리거나 줄인 뒤, 각 행의 슬롯에 씬에 배치한 버튼과(필요 시) 라벨 컴포넌트를 드래그해 넣으면 됩니다.
